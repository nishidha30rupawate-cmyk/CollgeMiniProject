  ...
  const loadOrders = async (isRefresh = false) => {
    ...
    try {
      const allOrders = await withTimeout(
        ...
        12000,
        "Admin orders request timed out. Please check your connection.",
      )
      setOrders(allOrders)
      setSelectedStatuses(
      ...
      const profiles = await Promise.allSettled(
        uniqueUserIds.map(async (userId) => {
          const profile = await withTimeout(
            ...
            6000,
            `Customer profile request timed out for user ${userId}`,
          )
      ...
      const profileEntries = profiles
        .filter((result): result is PromiseFulfilledResult<readonly [string, UserProfile | null]> => result.status === "fulfilled")
